...
class Matrix {
  constructor(rows, cols, data = []) {
    ...
    if (data === null || data.length === 0) {
      ...
      if (data.length !== rows || data[0].length !== cols) {
        throw new Error("Incorrecta dimension de datos!");
      }
    }
  }
  ...
  static checkDimensions(m0, m1) {
    if (m0.rows !== m1.rows || m0.cols !== m1.cols) {
      throw new Error("Matrices con diferentes dimensiones");
    }
  }
  ...
  static dot(m0, m1) {
    if (m0.cols !== m1.rows) {
      throw new Error('Matrices no son "dot"compatibles!');
    }
  ...
  // aply a function to each cell of the given matrix
